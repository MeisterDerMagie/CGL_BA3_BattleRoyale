    ...
    public class WatsonTcpClient : IDisposable
    {
        ...
        public void Connect()
        {
            ...
            if (_Mode == Mode.Tcp)
            {
                ...
                {
                    _Settings.Logger?.Invoke(Severity.Warn, _Header + "exception encountered: " + Environment.NewLine + SerializationHelper.SerializeJson(e, true));
                    _Events.HandleExceptionEncountered(this, new ExceptionEventArgs(e));
                    
                    //doodlenite
                    Debug.LogWarning("Could not connect to server provider.");
                    UnityThread.executeInUpdate(()=> ServerProviderClient.OnCouldNotConnectToServerProvider?.Invoke());
                    //
                    
                    //auskommentiert, weil Unity sonst einen Error anzeigt, der Error aber durch die obere Zeile abgefangen wird.
                    //throw;
                }
                finally
                {
